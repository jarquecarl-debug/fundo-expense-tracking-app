...
import { Plus, Trash2, Flag, RefreshCw } from "lucide-react";
...
/** Returns the next 1st-of-month date as YYYY-MM-DD */
...
export function ExpenseItemDialog({
  open, onClose, envelopeId, subcategoryId, item,
  ...
  // Recurring toggle — stored in item as item.recurring (bool)
  const [isRecurring, setIsRecurring] = useState<boolean>(
    (item as ExpenseItem & { recurring?: boolean })?.recurring ?? false
  );
  ...
  function onSubmit(values: FormValues) {
    ...
    } as Omit<ExpenseItem, "id"> & { recurring?: boolean };
  ...
  function handleClose() {
    ...
    setIsRecurring((item as ExpenseItem & { recurring?: boolean })?.recurring ?? false);
    onClose();
  }
  ...
  // Next reset date for display
  const nextReset = nextMonthFirst();
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex gap-4 text-sm bg-muted rounded-lg px-3 py-2">
              <div>
                ...
                <span className="font-medium">
                  ₱{estTotal.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                </span>
              </div>
              {actTotal !== null && (
                <div>
                  ...
                  <span className="font-medium text-primary">
                    ₱{actTotal.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField control={form.control} name="vendor" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g. Lazada, SM Store" {...field} data-testid="input-item-vendor" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="dueDate" render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    Due / Order By Date
                    {/* Suppress due date label hint when already Paid */}
                    {currentStatus === "Paid" || currentStatus === "Received" ? (
                      <span className="ml-1 text-xs text-muted-foreground font-normal">(ignored when paid)</span>
                    ) : null}
                  </FormLabel>
                  <FormControl>
                    <Input
                      type="date"
                      {...field}
                      disabled={currentStatus === "Paid" || currentStatus === "Received"}
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="low">
                        <span className="flex items-center gap-1.5">
                          <Flag className="w-3 h-3 text-blue-500" /> Low
                        </span>
                      </SelectItem>
                      ...
                      <SelectItem value="high">
                        <span className="flex items-center gap-1.5">
                          <Flag className="w-3 h-3 text-red-500" /> High
                        </span>
                      </SelectItem>
                    </SelectContent>
                  </Select>
            ...
            <FormField control={form.control} name="notes" render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea placeholder="Remarks, contact..." {...field} rows={2} data-testid="input-item-notes" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <div className="rounded-lg border border-border overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-sm font-medium leading-none mb-0.5">Recurring item</p>
                    <p className="text-xs text-muted-foreground">
                      Auto-resets status &amp; due date on the 1st of each month
                    </p>
                  </div>
                </div>
                {/* Toggle pill */}
              ...
              {/* Next reset info — shown when recurring is on */}
              {isRecurring && (
                <div className="flex items-center gap-2 px-4 py-2.5 bg-primary/5 border-t border-border">
                  <RefreshCw className="w-3.5 h-3.5 text-primary shrink-0" />
                  <p className="text-xs text-muted-foreground">
                    Next reset:{" "}
                    <span className="text-foreground font-medium">
                      {formatDateDisplay(nextReset)}
                    </span>
                    {" "}— status returns to{" "}
                    <span className="text-foreground font-medium">Unordered</span>
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                {totalPaid > 0 && (
                  <span className="text-xs text-green-600 dark:text-green-400 font-medium">
                    {formatPeso(totalPaid)} paid so far
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-[1fr_auto_1fr_auto] gap-2 items-end">
                ...
                <div>
                  ...
                  <Input
                    placeholder="optional" value={payNote}
                ...
                <Button
                  ...
                  className="h-8" onClick={addPayment} disabled={!payAmount}
                >
